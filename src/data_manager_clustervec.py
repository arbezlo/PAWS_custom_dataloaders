...
def init_data(
    ...
    if dataset_name == 'datasetcsv':
        return _init_datasetcsv_data(
            transform=transform,
    ...
    elif dataset_name == 'datasetcsv_fine_tune':
        batch_size = s_batch_size
        return _init_datasetcsv_ft_data(
            transform=transform,
            subset_path=subset_path,
            init_transform=init_transform,
...
#TODO create a _init_datasetcsv and datasetcsv_ft
def _init_datasetcsv_ft_data(
    transform,
    subset_path,
    init_transform,
    batch_size,
    ...
    imagenet = DatasetCsv(
        ...
        train=training,
        copy_data=copy_data)
    logger.info('DatasetCsv fine-tune dataset created')
    dataset = TransDatasetCsv(
        dataset=imagenet,
...
def _init_datasetcsv_data(
    ...
    datasetcsv = DatasetCsv(
        ...
        train=training,
        copy_data=copy_data)
    logger.info('DatasetCsv dataset created')
    unsupervised_set = TransDatasetCsv(
        dataset=datasetcsv,
        root= '/'.join((root_path, image_folder)),
        supervised=False,
        init_transform=init_transform,
    ...
    logger.info('DatasetCsv unsupervised data loader created')
    supervised_sampler, supervised_loader = None, None
    if classes_per_batch > 0 and s_batch_size > 0:
        logger.info('Making supervised DatasetCsv data loader...')
        supervised_set = TransDatasetCsv(
            dataset=datasetcsv,
            root="/".join((root_path, image_folder)),
            supervised=True,
            supervised_views=supervised_views,
        ...
        logger.info('DatasetCsv supervised data loader created')
...
def make_transforms(
    ...
    """
    #TODO create a datasetcsv case
    if 'datasetcsv' in dataset_name:
        logger.info('making datasetcsv data transforms')
        ...
        return _make_datasetcsv_transforms(
...
def _make_datasetcsv_transforms(
    unlabel_prob,
    training=True,
    basic=False,
    force_center_crop=False,
...
def make_multicrop_transform(
    ...
    if 'datasetcsv' in dataset_name:
        return _make_multicrop_datasetcsv_transforms(
            num_crops=num_crops,
            size=size,
            scale=crop_scale,
            normalize=normalize,
...
def _make_multicrop_datasetcsv_transforms(
    num_crops,
    size=96,
    scale=(0.05, 0.14),
    normalize=False,
...
class DatasetCsv(DatasetDataframe):
    ...
    def __init__(
        ...
        logger.info('Initialized DatasetCsv')

        
    #TODO override the original getitem to get the image_class


class TransDatasetCsv(DatasetCsv):
